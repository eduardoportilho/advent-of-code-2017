...
function getMax(registers) {
  let max
  for (let reg in registers) {
    if (max === undefined || registers[reg] > max)
      max = registers[reg]
  }
  return max
}

function runInstruction(instruction, registers) {
  const { reg, inc, val, condReg, condTest, condVal } = instruction
  if (registers[reg] === undefined)
    registers[reg] = 0
  if (registers[condReg] === undefined)
    registers[condReg] = 0

  const condRegVal = registers[condReg]
  const conditionMet = testCondition(condRegVal, condTest, condVal)
  if (conditionMet) {
    let nextRegVal = registers[reg]
    if (inc)
      nextRegVal += val
    else
      nextRegVal -= val

    registers[reg] = nextRegVal
  }
}

function testCondition(condRegVal, condTest, condVal) {
  if (condTest === '>')       {return condRegVal > condVal}
  else if (condTest === '<')  {return condRegVal < condVal}
  else if (condTest === '>=') {return condRegVal >= condVal}
  else if (condTest === '==') {return condRegVal === condVal}
  else if (condTest === '<=') {return condRegVal <= condVal}
  else if (condTest === '!=') {return condRegVal !== condVal}
...
function parseInstruction(text) {
  ...
  const reg = tokens[0]
  const inc = tokens[1] === 'inc'
  const val = parseInt(tokens[2])
  const condReg = tokens[4]
  const condTest = tokens[5]
  const condVal = parseInt(tokens[6])
  return {
    reg,
    inc,
    val,
...
const input = `koq inc 675 if xrh >= -6
it inc -402 if s < -1
g inc 283 if oj != -3
it inc 355 if y <= 5
do dec 553 if g != 288
