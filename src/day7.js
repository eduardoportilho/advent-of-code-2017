...
function solve(input) {
  const progs = parseProgs(input)
  const root = buildTree(progs)
  let {correctWeight, oddWeight} = findUnbalanced(root)
  console.log(`>>> ${oddWeight} should be ${correctWeight}`)
}
...
function findUnbalanced(root) {
  let parent = root
  while (true) {
    let {correctWeight, oddWeight, oddWeightNode} = getOddWeight(parent)
    if (isBalanced(oddWeightNode)) {
      return {correctWeight, oddWeight}
    }
    parent = oddWeightNode
  }
}

function isBalanced(node) {
  if (node.onTop.length === 0)
    return true

  const childWeight = node.onTop[0].totalWeight
  for (let child of node.onTop) {
...
function getOddWeight(node) {
  const wMap = {}
  for (let child of node.onTop) {
    if (!wMap[child.totalWeight]) {
      wMap[child.totalWeight] = []
    }
    wMap[child.totalWeight].push(child)
  }
  let correctTotalWeight, oddTotalWeight, oddWeightNode

  for (const weight in wMap) {
    if (wMap[weight].length === 1) {
      oddWeightNode = wMap[weight][0]
      oddTotalWeight = weight
    }
    else
      correctTotalWeight = weight
  }
  const delta = correctTotalWeight - oddTotalWeight
  // odd is less -> delta positive
  // odd is more -> delta negative

  const oddWeight = oddWeightNode.weight
  const correctWeight = oddWeight + delta
  return {correctWeight, oddWeight, oddWeightNode}
}

function buildTree(progs) {
  const rootName = findRootName(progs)
  const root = progs[rootName]
  const leafs = []
  let queue = [root]
  ...
  while (queue.length > 0) {
    let node = queue.pop()
    node.onTop = []
    if (node.onTopNames.length === 0) {
      leafs.push(node)
    }

    for (const onTopName of node.onTopNames) {
      let onTopNode = progs[onTopName]
      queue.push(onTopNode)
      // onTopNode.parent = node
      node.onTop.push(onTopNode)
    }
  }
...
function findRootName(progs) {
  const allNames = Object.keys(progs)
  let onTopNames = []
  for (const name in progs) {
    let prog = progs[name]
    onTopNames = onTopNames.concat(prog.onTopNames)
  }

  for (let name of allNames) {
    if (onTopNames.indexOf(name) < 0)
      return name
  }
  return null
}

function parseProgs(text) {
  const progs = input.split('\n').map(parseProg)
  const progMap = {}
...
function parseProg(text) {
  ...
  const weight = parseInt(text.match(/\((\d+)\)/)[1])
  let onTopNames = []
  if (text.indexOf('->') >= 0) {
    onTopNames = text.split('->')[1].trim().split(',').map(x => x.trim() )
  }
  return {name, weight, onTopNames}
}
